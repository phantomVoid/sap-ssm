...
import com.phantom.wss.quartz.QuartzProjectPackFeedIntf;
...
@Component
public class QuartzProjectPackFeedIntfImpl extends SapCommIntfImpl implements QuartzProjectPackFeedIntf {
    final Logger log = Logger.getLogger(this.getClass());

    final String funcName = "ZFM_MES_005";

    @Autowired
    @Qualifier("TProjectPackFeedDao")
    TProjectPackFeedDao packFeedDao;

    @Autowired
    @Qualifier("TPmProjectPostDao")
    TPmProjectPostDao postDao;

    @Autowired
    @Qualifier("TRfcLogDao")
    TRfcLogDao rfcLogDao;

    @Autowired
    @Qualifier("TProjectSapLogDao")
    TProjectSapLogDao sapLogDao;

    @Autowired
    @Qualifier("TProjectSapLogDetailDao")
    TProjectSapLogDetailDao sapLogDetailDao;

    @Autowired
    @Qualifier("TPmProjectBaseDao")
    TPmProjectBaseDao projectBaseDao;

    @Autowired
    @Qualifier("TQuartzLogDao")
    ...
    public String quartzExec() {
        ...
        try {
            ...
            for (TPmProjectPost projectPost : projectPostList) {
                ...
                criteria.andPROJECT_IDEqualTo(projectPost.getPROJECT_ID());
                criteria.andITEM_SNEqualTo(projectPost.getITEM_SN());
                criteria.andITEM_CODEEqualTo(projectPost.getITEM_CODE());
