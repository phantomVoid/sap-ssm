...
@Controller
@CrossOrigin
public class ProjectAccountCheckController extends SapCommIntfImpl {
    ...
    public String sync(@PathVariable("projectId") String projectId,HttpServletResponse response){
        String syncRes = syncByProjectId(projectId);
        ...
        for (SapAccountCheck accountCheck : projectList) {
            ...
            if (itOut.getNumRows() > 0) {
                for (int i = 0; i < itOut.getNumRows(); i++) {
                    ...
                    for (TAccountCheckInfo info : checkInfoList) {
                        BigDecimal itemNum = info.getITEM_NUM();
                        BigDecimal confirmOutput = checkInfo.getCONFIRM_OUTPUT();
                        BigDecimal curItemNum = checkInfo.getITEM_NUM();

                        if((itemNum.add(curItemNum)).compareTo(confirmOutput) == 0){
                            checkInfo.setITEM_NUM(confirmOutput);
                        }
                    }
                    ...
                    BigDecimal mesSum = checkBase.getMES_ITEM_SUM();
                    BigDecimal sapSum = checkBase.getSAP_ITEM_SUM();

                    if(mesSum.compareTo(sapSum) == 0){
                        checkBase.setCHECK_FLAG(Flag.Y.toString());
                        checkBase.setCHECK_DESC("校验通过");
                    }else{
                        checkBase.setCHECK_FLAG(Flag.N.toString());
                        checkBase.setCHECK_DESC("总量存在差异");
                    }
